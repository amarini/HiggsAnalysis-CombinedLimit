...
//////////////////////////////////////////////////////////////////////////////
//
// RooAbsMinimizerFcnSemiAnalytic is an interface class to the ROOT::Math function
...
#include "HiggsAnalysis/CombinedLimit/interface/RooAbsMinimizerFcnSemiAnalytic.h"
...
//#define oocoutW(A,B) cout
//#define oocoutI(A,B) cout

RooAbsMinimizerFcnSemiAnalytic::RooAbsMinimizerFcnSemiAnalytic(RooArgList paramList, RooMinimizerSemiAnalytic *context, bool verbose)
   : _context(context), _verbose(verbose)
{
  ...
  for (unsigned int i = 0; i < _floatParamList->size(); ) { // Note: Counting loop, since removing from collection!
    ...
    if (!arg->IsA()->InheritsFrom(RooAbsRealLValue::Class())) {
      ...
      cout << "RooAbsMinimizerFcnSemiAnalytic::RooAbsMinimizerFcnSemiAnalytic: removing parameter "
...
RooAbsMinimizerFcnSemiAnalytic::RooAbsMinimizerFcnSemiAnalytic(const RooAbsMinimizerFcnSemiAnalytic &other)
...
Bool_t RooAbsMinimizerFcnSemiAnalytic::synchronizeParameterSettings(std::vector<ROOT::Fit::ParameterSettings> &parameters, Bool_t optConst, Bool_t verbose)
{
   ...
   for (index = 0; index < _constParamList->getSize(); index++) {
      ...
      if (!par->isConstant()) {
         ...
         if (verbose) {
            ...
            cout
               << "RooAbsMinimizerFcnSemiAnalytic::synchronize: parameter " << par->GetName() << " is now floating." << endl;
         }
      }
      ...
      if (par->getVal() != oldpar->getVal()) {
         ...
         if (verbose) {
            //oocoutI(_context, Minimization)
            cout
               << "RooAbsMinimizerFcnSemiAnalytic::synchronize: value of constant parameter " << par->GetName() << " changed from "
   ...
   for (index = 0; index < _floatParamList->getSize(); index++) {
      ...
      if (!par->isConstant()) {
         ...
         if (!par->IsA()->InheritsFrom(RooRealVar::Class())) {
            //oocoutW(_context, Minimization) 
            cout
                                            << "RooAbsMinimizerFcnSemiAnalytic::fit: Error, non-constant parameter "
         ...
         if (pstep <= 0) {
            ...
            if (verbose) {
               //oocoutW(_context, Minimization)
               cout
                  << "RooAbsMinimizerFcnSemiAnalytic::synchronize: WARNING: no initial error estimate available for "
      ...
      if (par->isConstant() && !oldFixed) {
         ...
         if (oldVar != par->getVal()) {
            ...
            if (verbose) {
               //oocoutI(_context, Minimization)
                cout
                  << "RooAbsMinimizerFcnSemiAnalytic::synchronize: value of parameter " << par->GetName() << " changed from "
         ...
         if (verbose) {
            //oocoutI(_context, Minimization)
            cout
               << "RooAbsMinimizerFcnSemiAnalytic::synchronize: parameter " << par->GetName() << " is now fixed." << endl;
         }
         ...
         if (oldVar != par->getVal()) {
            ...
            if (verbose) {
               //oocoutI(_context, Minimization)
            cout
                  << "RooAbsMinimizerFcnSemiAnalytic::synchronize: value of fixed parameter " << par->GetName() << " changed from "
                  << oldVar << " to " << par->getVal() << endl;
            }
         }
         ...
         if (verbose) {
            ...
            if (oldVlo != pmin || oldVhi != pmax) {
               //oocoutI(_context, Minimization)
            cout
                  << "RooAbsMinimizerFcnSemiAnalytic::synchronize: limits of parameter " << par->GetName() << " changed from ["
            ...
            if (oldVerr != pstep && oldVerr != 0) {
               //oocoutI(_context, Minimization)
            cout
                  << "RooAbsMinimizerFcnSemiAnalytic::synchronize: error/step size of parameter " << par->GetName()
...
Bool_t
RooAbsMinimizerFcnSemiAnalytic::Synchronize(std::vector<ROOT::Fit::ParameterSettings> &parameters, Bool_t optConst, Bool_t verbose) {
   return synchronizeParameterSettings(parameters, optConst, verbose);
}

/// Modify PDF parameter error by ordinal index (needed by MINUIT)
void RooAbsMinimizerFcnSemiAnalytic::SetPdfParamErr(Int_t index, Double_t value)
{
   static_cast<RooRealVar*>(_floatParamList->at(index))->setError(value);
}

/// Modify PDF parameter error by ordinal index (needed by MINUIT)
void RooAbsMinimizerFcnSemiAnalytic::ClearPdfParamAsymErr(Int_t index)
{
   static_cast<RooRealVar*>(_floatParamList->at(index))->removeAsymError();
}

/// Modify PDF parameter error by ordinal index (needed by MINUIT)
void RooAbsMinimizerFcnSemiAnalytic::SetPdfParamErr(Int_t index, Double_t loVal, Double_t hiVal)
{
   static_cast<RooRealVar*>(_floatParamList->at(index))->setAsymError(loVal, hiVal);
}

/// Transfer MINUIT fit results back into RooFit objects.
void RooAbsMinimizerFcnSemiAnalytic::BackProp(const ROOT::Fit::FitResult &results)
{
...
Bool_t RooAbsMinimizerFcnSemiAnalytic::SetLogFile(const char *inLogfile)
{
   if (_logfile) {
      //oocoutI(_context, Minimization) 
      cout<< "RooAbsMinimizerFcnSemiAnalytic::setLogFile: closing previous log file" << endl;
      _logfile->close();
      delete _logfile;
      _logfile = 0;
   }
   ...
   if (!_logfile->good()) {
      //oocoutI(_context, Minimization) 
      cout<< "RooAbsMinimizerFcnSemiAnalytic::setLogFile: cannot open file " << inLogfile << endl;
      _logfile->close();
      delete _logfile;
      _logfile = 0;
   }
...
void RooAbsMinimizerFcnSemiAnalytic::ApplyCovarianceMatrix(TMatrixDSym &V)
{
...
Bool_t RooAbsMinimizerFcnSemiAnalytic::SetPdfParamVal(int index, double value) const
{
...
void RooAbsMinimizerFcnSemiAnalytic::printEvalErrors() const {
  if (_printEvalErrors < 0)
    return;

  std::ostringstream msg;
  if (_doEvalErrorWall) {
    msg << "RooAbsMinimizerFcnSemiAnalytic: Minimized function has error status." << endl
        << "Returning maximum FCN so far (" << _maxFCN
        << ") to force MIGRAD to back out of this region. Error log follows.\n";
  } else {
    msg << "RooAbsMinimizerFcnSemiAnalytic: Minimized function has error status but is ignored.\n";
  }
...
RooArgList *RooAbsMinimizerFcnSemiAnalytic::GetFloatParamList()
{
   return _floatParamList.get();
}
RooArgList *RooAbsMinimizerFcnSemiAnalytic::GetConstParamList()
{
   return _constParamList.get();
}
RooArgList *RooAbsMinimizerFcnSemiAnalytic::GetInitFloatParamList()
{
   return _initFloatParamList.get();
}
RooArgList *RooAbsMinimizerFcnSemiAnalytic::GetInitConstParamList()
{
   return _initConstParamList.get();
}

void RooAbsMinimizerFcnSemiAnalytic::SetEvalErrorWall(Bool_t flag)
{
   _doEvalErrorWall = flag;
}
void RooAbsMinimizerFcnSemiAnalytic::SetPrintEvalErrors(Int_t numEvalErrors)
{
   _printEvalErrors = numEvalErrors;
}

Double_t &RooAbsMinimizerFcnSemiAnalytic::GetMaxFCN()
{
   return _maxFCN;
}
Int_t RooAbsMinimizerFcnSemiAnalytic::GetNumInvalidNLL() const
{
   return _numBadNLL;
}

Int_t RooAbsMinimizerFcnSemiAnalytic::evalCounter() const
{
   return _evalCounter;
}
void RooAbsMinimizerFcnSemiAnalytic::zeroEvalCount()
{
   _evalCounter = 0;
}

void RooAbsMinimizerFcnSemiAnalytic::SetVerbose(Bool_t flag)
{
   _verbose = flag;
}

void RooAbsMinimizerFcnSemiAnalytic::setOptimizeConst(Int_t flag)
{
   ...
   if (_optConst && !flag) {
      if (_context->getPrintLevel() > -1)
         //oocoutI(_context, Minimization) 
             cout << "RooAbsMinimizerFcnSemiAnalytic::setOptimizeConst: deactivating const optimization" << endl;
      setOptimizeConstOnFunction(RooAbsArg::DeActivate, true);
      _optConst = flag;
   } else if (!_optConst && flag) {
      if (_context->getPrintLevel() > -1)
         //oocoutI(_context, Minimization) 
         cout << "RooAbsMinimizerFcnSemiAnalytic::setOptimizeConst: activating const optimization" << endl;
      setOptimizeConstOnFunction(RooAbsArg::Activate, flag > 1);
      _optConst = flag;
   } else if (_optConst && flag) {
      if (_context->getPrintLevel() > -1)
         //oocoutI(_context, Minimization) 
         cout<< "RooAbsMinimizerFcnSemiAnalytic::setOptimizeConst: const optimization already active" << endl;
   } else {
      if (_context->getPrintLevel() > -1)
         //oocoutI(_context, Minimization) 
         cout << "RooAbsMinimizerFcnSemiAnalytic::setOptimizeConst: const optimization wasn't active" << endl;
   }

   RooAbsReal::setEvalErrorLoggingMode(RooAbsReal::PrintErrors);
}

void RooAbsMinimizerFcnSemiAnalytic::optimizeConstantTerms(bool constStatChange, bool constValChange) {
   if (constStatChange) {

      RooAbsReal::setEvalErrorLoggingMode(RooAbsReal::CollectErrors) ;

      //oocoutI(_context,Minimization) 
      cout << "RooAbsMinimizerFcnSemiAnalytic::optimizeConstantTerms: set of constant parameters changed, rerunning const optimizer" << endl ;
      setOptimizeConstOnFunction(RooAbsArg::ConfigChange, true) ;
   } else if (constValChange) {
      //oocoutI(_context,Minimization) 
      cout << "RooAbsMinimizerFcnSemiAnalytic::optimizeConstantTerms: constant parameter values changed, rerunning const optimizer" << endl ;
...
bool RooAbsMinimizerFcnSemiAnalytic::getOptConst()
{
   return _optConst;
}

std::vector<double> RooAbsMinimizerFcnSemiAnalytic::getParameterValues() const
{
