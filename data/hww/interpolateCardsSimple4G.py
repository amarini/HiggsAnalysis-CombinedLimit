...
for X in [ 'SM4_hwwof_0j_shape',  'SM4_hwwof_1j_shape',  'SM4_hwwsf_0j_shape',  'SM4_hwwsf_1j_shape', 'SM4_hww_2j_cut']:
    ...
    if "shape" in X:
        ...
        ofile  = ROOT.TFile( "%s/%g/%s%s.input.root" % (options.ddir,mass,XS,options.postfix) , "UPDATE" )
    ...
    shapeLines.append( ("*",        obskeyline[0], [ "%s%s.input.root" % (X,options.postfix),  "histo_$PROCESS" ]) )
    shapeLines.append( ("data_obs", obskeyline[0], [ "%s%s.input.root" % (X,options.postfix),  "histo_Data"     ]) )
